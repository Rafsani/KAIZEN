      ...
      if (authQueryresult.status == "OK") {
        output = loanQueryResult.data.filter((item) => {
          ...
          item.contracts.every((element) => {
            if (_.isEqual(element.lenderId, authQueryresult.data)) {
              // using lodash to check equality between two objects
              retValue = false;
...
const handlePOSTCreateLoan = async (req, res, next) => {
  try {
    ...
    if (authQueryresult.status == "OK") {
      ...
      formQueryResult = await authInterface.checkIfFormFilled(
        authQueryresult.data
      );

      if (
        !formQueryResult.data.hiddenDetails ||
        !formQueryResult.data.collateral
      ) {
        return res.status(400).send(formQueryResult);
      }

      loanQueryResult = await loanInterface.getLoanByUserID(
        authQueryresult.data,
        "Pending"
      );
...
const handleGETPendingLoanForUser = async (req, res, next) => {
  try {
    const lastIssuedLoan = await loanInterface.lastIssuedLoan(
      req.params.userId
    );

    const loanQueryResult = await loanInterface.getLoanByUserID(
      req.params.userId
    );
    ...
    if (loanQueryResult.status == "OK") {
      ...
      if (data.contracts[0] != null) {
        ...
        data.contracts.forEach((element) => {
          let minDate = checkInstallmentDate.returnNextInstallmentDate(
            element.installmentDates
          );
      ...
      let outputInstallmentDate =
        minInstallmentDate.getTime() == new Date(8640000000000000).getTime()
          ? "No Contract Signed Yet"
          : minInstallmentDate.getDate() +
            "/" +
            parseInt(minInstallmentDate.getMonth() + 1) +
            "/" +
            minInstallmentDate.getFullYear();
      let outputExpirationDate = new Date(
        new Date(data.issueDate).getTime() + parseInt(process.env.EXPIRE_LOAN)
      );
...
const handleGETLoanOffers = async (req, res, next) => {
  try {
    const loanQueryResult = await loanInterface.getLoanOffersByUserID(
      req.params.loanId,
      "offerRequests"
    );
    let outputOffers = [];
    ...
    if (loanQueryResult.status == "OK") {
      ...
      loanOffers.forEach((contractRequest) => {
        ...
        let expirationDate =
          contractRequest.installmentDates[
            contractRequest.installmentDates.length - 1
          ];
  ...
  try {
    ...
    if (loanQueryResult.status == "OK") {
      ...
      currentLenders.forEach((contractRequest) => {
        ...
        let nextInstallmentDate = checkInstallmentDate.returnNextInstallmentDate(
          contractRequest.installmentDates
        );
        let installmentAmount = Math.floor(
          (totalAmount * (1 + contractRequest.interestRate / 100)) /
            contractRequest.installments
        );
        let installments = contractRequest.installments;
      ...
      if (outputOffers.length != 0) {
        ...
        outputOffers.sort((a, b) => {
          return (
            a.nextInstallmentDate.getTime() - b.nextInstallmentDate.getTime()
          ); // ascending order
        });
...
const handlePUTRepayLoans = async (req, res, next) => {
  try {
    const loanQueryResult = await loanInterface.repayLoan(
